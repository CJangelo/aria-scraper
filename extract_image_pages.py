...
def detect_column_positions(rows: list[list[dict]],
    ...
    Returns sorted list of column x-positions, or None if no table.
    """
    if len(rows) < 3:
        return None
...
def rows_to_markdown_table(rows: list[list[dict]],
    ...
    """Convert grouped rows into a Markdown table."""
...
def run(pdf_path: Path) -> dict:
    """
    OCR all image pages for a PDF.  Returns a result dict:
    ...
    # Initialize OCR engine (once per call — reused across pages)
