...
def main():
    ...
    print()
    print("Sub-documents found:")
    print("-" * 60)
    for rid, sd in manifest["sub_documents"].items():
        label = sd["document_type_hint"] or "unknown"
        print(
            f"  RefID {rid}: pages {sd['first_page']}–{sd['last_page']} "
            f"({sd['page_count']} pp, {sd['image_pages']} image) [{label}]"
        )
    print("=" * 60)
